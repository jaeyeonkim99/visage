...
from modeling.visage import VisageConfig, VisageForAmbisonicsGeneration
...
def main():
    ...
    if hasattr(args, "model_path") and args.model_path is not None:
        ...
        model = VisageForAmbisonicsGeneration.from_pretrained(
            args.model_path, config=config, ignore_mismatched_sizes=True
        )
        model.init_spatial_from_pretrained()
        logger.warning(config._attn_implementation)
        logger.warning(f"Loading pretrained model from {args.model_path}")
        logger.warning(f"Initialized spatial channels from W channel")
    else:
        config = VisageConfig.from_pretrained(args.model_cfg)
        model = VisageForAmbisonicsGeneration(config=config)
    ...
    preprocessor = PreprocessorAmbi(
        ...
        energy_map_path=args.energy_map_path,
        rotation_base_path=args.rotation_base_path,
...
def validation_loss(
    ...
    validation_loss = 0
    codebook_loss_list = [0 for i in range(config.num_rvq * 4)]
    ...
    if args.with_tracking:
        ...
        for i in range(config.num_rvq * 4):
            ...
            with torch.autocast("cuda", torch.float16):
                if config.classifier_free_guidance:
                    generated_codes = accelerator.unwrap_model(model).generate_cfg(
                        ...
                        batch["direction"].cuda(),
                        batch["energy_map"].cuda(),
                        do_sample=True,
                        top_k=256,
                    )
                else:
                ...
                for i in range(4):
                    ...
                    tb_tracker.add_audio(
                        tag=f"samples/{vid}_channel_{i}",
                        snd_tensor=audio,
                        global_step=logging_step,
                        sample_rate=config.dac_sample_rate,
                    )
                cos = {}
                cos[f"valid/cos_{vid}_WY"] = (
                    similarity(latents[0], latents[1]).mean().item()
                )
                cos[f"valid/cos_{vid}_XY"] = (
                    similarity(latents[1], latents[3]).mean().item()
                )
                accelerator.log(cos, step=logging_step)
    model.train()
